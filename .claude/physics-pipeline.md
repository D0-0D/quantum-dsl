...
- **sung**(论文对照, PRX 11 021058)——**已升格为契约 N15**(fixture 进
  ...
  3 浮动双岛 transmon。可断言: C_Σ ×3 对论文 **±8%**、β_qc ±20%; 结构性排除:
  C_12/β_12(本简化版图无直接 q-q 路径, 差分远场相消, Elmer 也只得 2%)、
  绝对 g 与可调 qubit 的 f01(论文在磁通工作点, 模型坐零磁通)。
  **2026-08-25 整片 o2 首测翻案**(原判据 ±5% 按 Elmer 档标定):
  契约配方(80/2, o2, 14.1M tets / 18.5M 未知量)实测 C_Σ =
  95.8/220.7/95.8 fF = 论文 ×0.965/0.969/0.940; **同网格 order 1 =
  101.4/232.6/101.3, 对 Elmer 档(P1)<1%**——两独立求解器同 P1 同答案
  证明管线正确, 同时揭穿 Elmer 档"对论文 1.00–1.03"是 P1 正偏
  (+5.4~5.8%, 与 two_pads o1 +7.3% 同类)与简化版图结构性缺失
  (−4~6%: 无 junction leads/读出爪/控制线)**相互抵消**的产物——
  上一段"不是抵消产物"的旧判断作废。版图对称 ⇒ QB1/QB2 必然同值,
  论文两值(99.3/101.9)不可能同时命中。β_qc o2 实测 0.0393(+8%)。
  网格灵敏度: 160/10 粗档给 101.2/231.0/101.1(+1.4~1.9% 对论文)——
  粗细两档跨 ~5.5%, 契约配方仍未收敛, 更细只会更低(离散从上方收敛)。
  ⚠ v3 自己的 Palace "分块 o2 对 Elmer −3%" 同样是抵消产物: 同 order 下
  分块抬 C_Σ +12~15%, order 1→2 压 −20~22%, 净 −3%(v3 session 2607290329
  原话"两个大误差反向抵消"); v3 整片 o2 因 #22 从未跑成。
...
- **每个 terminal 解完都跑误差估计器**(RT 通量恢复), 即使
  ...
  **N15 规模实测**(2026-08-25, sung 80/2 = 14.1M tets, o2 18.5M H1 未知量,
  estimator 空间 ND 88M / RT 125M): np=32 峰值内存 **154G**(64C/128G 机
  在多重网格层级组装处被 OOM 杀, signal 9 无一字诊断——大网格失败先想
  内存), 96C/384G 机 20 min 完成(其中串行 mesh 预处理 6 min + 盘 IO
  4 min; o1 同网格 11 min)。Palace 自报的"current memory"在组装前打印,
  远低于后续峰值, 不能当预算依据。
