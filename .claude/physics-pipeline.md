...
C 矩阵到哈密顿量(V4-3, 已对契约 golden 逐位验证的公式口径):

- 对称化 `(C+Cᵀ)/2` 后求逆(求解器数值噪声不该漏进物理量)。
- `C_Σ,i = 1/[C⁻¹]_ii`, `E_C = e²/(2C_Σ)`——**不是** Maxwell 对角元, 逆矩阵
  对角才是结点看到的总电容(N8 有反例测试)。
- `E_J = (ħ/2e)²/L_J`; transmon 微扰式 `f01 = (√(8E_C·E_J) − E_C)/h`, `α = −E_C/h`。
- 耦合 `β = |C⁻¹_ij|/√(C⁻¹_ii·C⁻¹_jj)`, `g = ½·β·√(f01_i·f01_j)`(v3/契约口径,
  golden 复算差 <1e-10; 文献里也有用裸等离子频率的口径, 差 ~E_C/h 量级——契约
  钉了前者, 别"顺手改进")。
- SQUID: `E_J(φ) = (E_J1+E_J2)·hypot(cos πφ, d·sin πφ)`, `d=(E_J1−E_J2)/(E_J1+E_J2)`
  ——无奇点式(tan 写法在 φ=0.5 除零)。
- 常数用 SI-2019 精确值(e=1.602176634e-19 C, h=6.62607015e-34 J·s)。
- CPW 解析(N10)与 χ/等效 LC(N11): v3 `cpw_analytic.py` 与
  `circuit_model.py` 的对应函数在 py3.13 下对 golden 命中 1e-16/精确——
  **审后原样搬运**即可, 无需重推。
...
two_pads 全部实测(golden = `[[24.7288,−1.976],[−1.976,24.7293]]` fF ± 2%):

| # | 表示 | 网格 max/min µm | order | C_AA (fF) | C_AB (fF) | 对 golden 最大偏差 |
|---|---|---|---|---|---|---|
...
| C | **(a) 零厚度片** | 40/4 | 2 | 24.532 | −1.947 | 1.46% ✓ |
...
- **2 µm 板厚效应是真实物理, 不是离散误差**: 同网格下 (b)−(a) 差
  ...
  (a) 才是更忠实的模型。
- 因此 C′ 的 "FAIL" 不是 (a) 的缺陷, 而是 golden 绑定了 v3 的 2 µm 板 +
  粗网格两个工件偏置——见 §7 的契约决议。
...
N7 golden 钉的是 v3 配方(2 µm 板 + 40/4 网格)的产物。实测(§6)显示:
方案 (a) 在 40/4 下 1.46% 勉强通过, 但其**收敛值离 golden 2.9%**——golden
与零厚度物理之间有 ~2–3% 的真实差(板厚 + 网格偏置), 不是实现质量问题。
两个自洽的选项:

1. **V4-2 默认 (a) + 网格参数照抄 fixture(40/4, order 2)**: 现契约直接过
   (1.46% < 2%), 实现最短; 但余量薄(~0.5%), 且"物理上更收敛反而离 golden
   更远"这个倒挂长期存在。
2. **用 (a) 的实测值重钉 `LIVE_MAXWELL_GOLDEN = [[24.532, −1.947], [−1.947,
   24.535]]`(40/4, order 2 配方)**: 契约是 v4 自己的, golden 的意义在
   "可复现的实测锚点"而非"2 µm 板这个历史工件"。之后余量恢复 ~0.04% 级,
   锚点与 v4 产品路径同配方。N7 第二条 f01 golden(容差 3%)受影响 ~+0.4%,
   仍在容差内, 可不动(或一并重算)。

推荐 2, 但这改契约数值, **由用户拍板**; 拍板前 V4-2 按 1 实现即可, 切换
成本是一组常数。
...
6. 输入卫生: CSV 与 L_J/E_J 拒 NaN/Inf(`not (isfinite(v) and v>0)`——1e400
   溢出成 inf 能绕过 `<=0`); C 求逆前对称化 + 尺度相对的奇异判据。
...
外部证据(Palace 官方示例的金属表示 / 业界零厚度 vs 膜厚惯例 / 膜厚对 fF 级
电容的量化影响 / L0 与输出约定的官方定义)已派 codex(gpt-5.5+web_search)
取证, job: `~/.codex-jobs/20260824-070412-palace-metal-modeling/answer.md`。
结论回填至此节。
