...
超导量子芯片版图 DSL —— native Gmsh `.geo`（µm）**或**版图 `*.layout.yaml`（模板 `.geo` + yaml 接口，与手写 `.geo` 同一 gmsh 模型混用）+ `*.meta.yaml` →
...
**v4.0**（2026-08-26）+ **版图编排**（2026-09-12）+ **Chen 2025 圆盘比特 3×3**（2026-09-13）。金属用**零厚度片 imprint** 表示（不是 v3 的挖空），整条链 ≈3 000 行；
...
python -m pytest tests/ -q                  # 51 passed, 2 skipped (~3 min)
...
- `v4-dev` —— v4 的 TDD 开发痕迹与原始调研存档（session logs、文献取证、sung 原始 CSV、原型脚本）。
