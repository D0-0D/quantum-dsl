...
| targets（issue #23） | meta `targets: {source, qubits: {Q: {<字段>: 值, tol: 8%}}, couplings: [{pair: [a, b], beta│g_MHz: 值, tol}]}`（键 = results.yaml hamiltonian 的数值字段名, 值裸数, tol 只收百分比）→ `build(solve=True)` 整片 `results.yaml` 多 `validation: {source, passed, checks[{qubit│pair, field, expected, tol, actual, deviation, pass}]}`（deviation = 实测/期望 − 1）; 未命中只 warn; 拼错 qubit 名在网格化之前 raise; 分块结果不核对（warn） | `test_frontend.py::test_load_meta_targets_flattened_and_strict`、`test_pipeline.py::test_targets_validation_in_results` |
| 网格收敛（issue #26） | `converge(meta, out, scales=(1.0, 0.6)) → {runs, convergence, doc}`: `mesh.max/min_size_um` 同乘每个 scale, 各档整片 `build(solve=True)` 落 `out/mesh_x<s>/`; `convergence.yaml` 记最细两档每个 Maxwell 元素与 hamiltonian 数值字段的相对变化（细 − 次细）/|细| | `test_pipeline.py::test_converge_scales_mesh_and_diffs_finest_two`、`test_live.py::test_two_pads_converge` |
| sung 外部锚（外部物理验证） | sung 例子（PRX 11.021058）live 解: C_Σ ×3 对论文 ±8%（2026-08-25 翻案: 原 ±5% 锚的 Elmer P1 档系偏置抵消产物, 账见例子 meta 头注与 `docs/physics.md` §12）, β_qc ±20%（判据 = 例子 meta 的 `targets:`, 测试锁死核对集合与容差; gate `QDSL_RUN_PALACE_SUNG=1`; 排除项见例子 meta） | `test_live.py::test_sung_2021_against_paper` |
...
| 自动布线 | `route.plan_cpw(start, end, R, length, region, n_legs, width, lead, axis) → [("line", …) \| ("arc", …)]`、`route.auto_radius(…) → R`（纯 math，不拉 gmsh）：两端各先沿端口法向直走 `lead`；**骨架**默认走曼哈顿框架（横平竖直，`axis` 可转角；`axis: free` = Dubins CSC 四型），端弧 + 直 / Z / U / L / 三折模板，按（转弯数, 长度）排序，出区域或自身两段太近（沿路间隔 ≥ πR 的原语 < 2R）的丢弃；`length` 时在骨架直段上放蛇形，**每个弯按该处余量独立外推（腿可不等长、随沿轴可用宽度变）**，余量精确（半圆取样点射线 + 界的角点 / 端点射进加厚半圆 + 两弧外切；界 = region 外框 / 缝 / 自身直段与弧 / 已放的块，净距 2R），腿数自动取最小可行；**单段装不下时按容量比例分摊到多段**（容量小的先放，先放的块进后面块的障碍集，拐角处不交叉；`n_legs` = 总腿数）；`R: auto` = 整数二分的最大可行弯半径（给定 R 装不下时提示能装下的最大 R）；`region` = 矩形或**矩形并集**（外框取样点精确 + 缝的精确距离，含缝宽）；弧段 ≤ π/2、G1 连续、记账 == 目标；输出前自检区域 / 净距 / 记账；两口正对共线且无 region 时与 `cpw_meander` 逐段相同；装不下 / 出界 / 非法输入 raise 并逐骨架说明。模板 `lib/cpw_route`（`planner: cpw`，两口不必正对，位姿恒等，原语按列表变量注入 `.geo`）；步骤键 `region:` / `axis:`（仅 planner 模板）；`mirror:` 对 planner raise；`subsystems` 记 `route_primitives` 与 `R_um`；例子 `cpw_route_demo`（四对 xmon：横平竖直 / 拼接区域绕路 / 自由角 / R 自动） | `test_route.py` 11 条（曼哈顿 L + 不等腿；拼接区域绕路 + 缝检查；转框架 + 自身净距 + axis 错误；多段分摊 + 拐角净距；逐弯精确余量（台阶肩部阈值 841.16）；自身净距淘汰；R 自动 = 70 / 提示；自由角四型 / G1 / 记账 / lead / 精确包围盒 / raise；退化一致性；纪律；demo 四条路 + GDS 落在 region 并集内） |
...
1. Python 3.13 下 `pytest tests/ -q` **0 failed**（54 passed, live 三条默认 skip; targets / 收敛加 3; v4.0 收口时 39, 版图编排 / Chen / 自动布线各加, 自动布线 v2 加 3, 多段分摊 / 精确余量 / 净距 / R 自动加 4）。
