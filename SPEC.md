...
| 自动布线 | `route.plan_cpw(start, end, R, length, region, n_legs, width) → [("line", …) \| ("arc", …)]`（纯 math，不拉 gmsh）：Dubins CSC 四型取最短可行、弧段 ≤ π/2、G1 连续；`length` 时中段换蛇形（与 `LIB_CPW_MEANDER` 同闭式，**两口正对共线时与 `cpw_meander` 逐段相同**）；`region` 矩形校验含缝宽；装不下 / 出界 / 非法输入 raise。模板 `lib/cpw_route`（`planner: cpw`，两口不必正对，位姿恒等，原语按列表变量注入 `.geo`）；步骤键 `region:`（仅 planner 模板）；`mirror:` 对 planner raise；例子 `cpw_route_demo` | `test_route.py` 4 条（规划器四型 / G1 / 记账 / raise；退化一致性；纪律；demo 集成 + GDS 落在 region 内） |
...
1. Python 3.13 下 `pytest tests/ -q` **0 failed**（44 passed, live 两条默认 skip; v4.0 收口时 39, 版图编排 / Chen / 自动布线各加）。
