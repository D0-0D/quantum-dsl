...
11. Before checkpointing the connection-pad generator as TransmonPocket-parity complete, align qlibrary behavior for:
    - generated connection-pad pin `gap`: original `TransmonPocket` does not pass `gap`, so `QComponent.add_pin()` uses `0.6 * cpw_width`;
    - symbolic default `cpw_width` and `cpw_gap`: original qlibrary resolves them from Metal design variables, so YAML `connection_pads: {readout: {}}` must build without explicit root `vars`.
12. Add two-transmon YAML example.
13. Add parity and no-qlibrary-construction tests.
14. Update README with the new authoring path.
