...
two_pads 的 live golden 是**可复现的实测锚点，不是绝对真值**：
...
**参数**：
- `C_Σ,k = 1/[C′⁻¹]_θθ,kk`（dressed 有效电容，**不是** Maxwell 对角元——LOM 论文 [L1] Eq. 8/11a 同口径；电路模型有反例测试：
...
- **χ（子系统）**：`chi_0 = −2g²·f01/(f01² − f_r²)`，`chi_1 = g²·(1/(f01−f_r) − 2/(f12−f_r) + 1/(f01+f_r) − 2/(f12+f_r))`，
...
## 12. 外部物理验证：Sung et al. PRX 11, 021058（sung 外部锚）
...
## 13. flip-chip 口径与 Chen 2025 QCQ 首解
...
⚠ **读这张表前先看块的边界条件**：块只保留自己的 6 个 component，块外的一切在 `build_mesh` 里当孤儿删掉——包括**块内比特伸向块外耦合器的爪**。
H01 块丢了 5 只（Q01.N→V01、Q01.S→V00、Q11.E→H11、Q11.N→V11、Q11.S→V10），每只约 6944 µm²、距盘边 12 µm，而载片地只隔 5 µm。
真器件里这些爪是比特最近邻的导体之一，所以表中的对地项（C01/C02）**系统性偏低**，互容缺口的归因也因此不是干净的。
量化它要么整片解（42 导体，上云），要么在块里保留邻居爪并把它们 Schur 消掉；两者都还没做。下面的读法都受这条限制。

读法：① d 只动对地项（近似 ∝ 1/d），**几乎不动互容**——互容的一半缺口是几何（缝 70 / 盘–爪 12 / 条–板 75 都是照片 B/C 级量）或 SI 电磁模型口径，
或上面那条缺爪的系统偏差，不能靠拟合 d 吸收；② 派生量 E_C(q) 220 → 259 MHz、g_qc 66 → 85 MHz（d 5 → 7）夹住了 SI 自身闭合值（209 / 71）与设计值（185 / 90），
...
`build()` 现在会对重叠的块集合 warn（`_warn_overlapping_blocks`），同时点出「块外导体被丢弃」这条。
账与产物：`.claude/session/2609131337.md`、`.claude/chen-evidence/`；几何出处 `design/paper-chen2025-geometry.md`。

### 13.1 十字整片解: 环境效应、d 标定与实测 α（2026-09-18）

块解的「删爪」偏差用 `examples/chen_2025_cross`（中心 Q11 四爪全开 + 四臂比特各留一只朝中心的爪 + 4 耦合器 = 18 terminal, 无分块）整片一次解量化
（c24a1 64 核 / 128 G, 100/4 order 2, 334 万未知量, np = 24 约 19 min, 峰值 28 GB; 产物 `.claude/chen-evidence/cross_d{4,5,7}_100_4_*`, 账 `.claude/session/2609180210.md`）:

| d (µm) | 中心 Q11 C_Σ / E_C | 臂比特 (单爪) E_C | 耦合器 E_C | g_qc | g_qq | 互容 半盘–半盘 / 半盘–条 / 条–板 |
|---|---|---|---|---|---|---|
| 4 | 101.6 fF / **190.7 MHz** | 194.5 | 137.2 | 55 | 2.6 | 9.5 / 12.3 / 9.7 |
| 5 | 90.3 / 214.5 | 219.9（= H01 块解 220） | 154.2 | 65–67 | 3.7 | 10.2 / 12.7 / 10.7 |
| 7 | 77.3 / 250.4 | 259.0（= 块解 259） | 180.6 | 84–85 | 5.8 | 11.3 / 13.5 / 12.2 |
| 参考 | 实测 α −179～−200 MHz（12 只, 均值 **−192 ± 6**）; SI 网络自身闭合 209; 设计 −185 | — | SI 闭合 171 | 设计 90 / SI 闭合 71 | 设计 5 / SI 闭合 9.1 | SI 23 / 26 / 28 |

读法：① **环境效应小**：四配位只把 E_C 从 219.9 压到 214.5（−2.5%）——多出的三只爪主要把半盘的场从「对地」改道成「对耦合器互容」（d = 5 时中心半盘对地 133 fF vs 臂 145 / 153），
C_Σ 净增 2.2 fF；臂比特与 QCQ 块解到 0.1 MHz 重合，证明块解 = SI 孤立 QCQ 口径、删爪偏差只落在多爪比特上。
② **d 是唯一旋钮且能命中**：三点给 E_C ∝ d^0.49；d = 4.0 µm 时中心比特 α = −190.7 MHz 落在实测均值 ±6 内（f01 = 4.16 GHz, 实测 f_max 4.03–4.25）。
但 d 的外部锚是 BAQIS 同工艺实测 5 ± 0.4 µm，本标定差 2.5σ；SI 自己的电容网络对应 E_C 209 ↔ d ≈ 5.2，与外部锚一致——即**我们的几何在同一 d 下比 SI 的电磁模型少约 10% 的 C_Σ**，
缺口来源按大小排：未画读出爪（估 +3% C_Σ, 同方向）、10–15 µm 级间隙的照片误差（互容只有 SI 的 0.4–0.5 倍, 与 d 无关）、单网格未收敛（100/4 与 80/3 两档见下）。
③ 派生量在 d = 4 处 g_qc = 55 MHz（设计 90, SI 闭合 71）、g_qq = 2.6 MHz（设计 5）系统偏低——它们由互容主导，互容缺口不能靠 d 吸收，是几何稿里那几个 C 级量（缝 70、盘–爪 12、条–板 75）的账。
④ 报数口径：**用一个量（实测 α）定 d，其余全部是预测**——这正是 `design/paper-chen2025-geometry.md` §5.2 定的规矩。
⑤ **3×3 整片一次解**（42 terminal, d = 5, 100/4, 812 万未知量, np = 32 86 min, 峰值 69 GB, `full3x3_d5_100_4_*`）与十字互证：中心 Q11 E_C 214.5 MHz 与十字到 0.1 MHz 相同；
配位数每多一只爪 E_C −1.8 MHz（角 218.1 / 边 216.3 / 中心 214.5）；对角次近邻的直接 g 只有 0.03–0.05 MHz（半盘–半盘互容 7×10⁻⁴ fF），
所以 SI §G 实测的 ~2 MHz 次近邻残余耦合不是静电直接项。结论：比特级量用十字就够，3×3 只在阵列级耦合上多给信息。
⑥ **网格两档**（十字 d = 5, 100/4 → 80/3, 239 万 → 502 万 tets, 334 万 → 694 万未知量）：对地 C −0.1～−0.3%，互容 −0.5～−1.1%，E_C +0.3%，g_qc −0.6%，g_qq −1.3%
——远小于 d 三点间距与 12% 口径缺口，上表按 100/4 报数成立（`cross_d5_80_3_results.yaml`）。
...
- **内存预算与旋钮**（2026-09-18 c24a1 64 核 / 128 G 实测，Chen 十字 / 3×3，order 2）：峰值 ≈ **8.1–8.6 GB / 百万未知量**（np 24–32；3.34 M → 28.6 G，6.94 M → 59.0 G，8.12 M → 69.2 G，2.82 M → 25.7 G），
  Palace 0.16 末尾自报的 "Estimated peak per-node memory usage" 与外部采样一致，可当预算依据（开头的 "current memory" 不行）。0.16 默认 `PartialAssemblyOrder = 1`（p = 2 层已是 PA），
  求解器侧旋钮很弱：np 32 → 16 只省 8%（时间 ×1.7），`Linear.AMGAggressiveCoarsening` 只省 5%；**杠杆在网格**：3×3 的 `min_size_um` 4 → 6（100/4 → 150/6）内存 ÷2.7、时间 ÷2.6，
  代价 E_C −0.6%、互容 +1～2%。128 G 机可装 ~14 M 未知量。
...
- **版本**：最新 0.17.0（2026-06）；spack 只到 0.16.0（本机同版）。0.16.1 / 0.17 带来的是诊断（分阶段内存表、resolved
  config）与 cracking / AMR 修复，**无静电相关行为变化**——留在 0.16 没有已知代价；若升，从源码 tag 构建并用 two_pads golden 回归。
