...
| [`04-论文验证闭环-sung.md`](04-论文验证闭环-sung.md) | **说明用**。对 PRX 11.021058 的验证到底验证了什么：两块几何——经 Elmer 标定的替代几何 `sung_2021_device`（N15 测的）与从照片量出的真拓扑 `sung_2021_xmon`（零调参预测）；论文真值怎么读、闭环每一步、照片测量方法与误差预算、Elmer 旁路与同网格交叉、order 1/2、为什么 ±5%→±8% 是两个误差相抵、为什么真拓扑不调参就差 12～30% | 想弄清「对论文 ±8%」含义与边界、以及本仓预测精度上限的人 |
