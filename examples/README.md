...
| `chen_2025_3x3.{layout.yaml,meta.yaml}` | Chen et al. 2025 (Nat. Phys. 21, 1489) 圆盘比特方格阵列的 3×3 切片: 9 个 `disc_transmon` + 12 个 `bar_coupler`, 几何全部是 Fig. 1a 照片量出值(`../docs/design/paper-chen2025-geometry.md`), 无读出结构 | flip-chip 口径: 顶片无地, `ground: none`, 载片地 = `airbox.top_um: 5`(唯一物理旋钮); 21 个浮动结条目自动生成; `extract.blocks` = 12 个 SI §D 口径的孤立 QCQ 块(6 terminal), 可逐项对 SI 的 11 个电容 | 单块 100/4 order-2 ≈75 万 tets, 本机 8 rank 可解 —— 但要自己 `compile_layout(m).for_block(comps)` 再 `build_mesh`: `build()` 会先给整片出网格(~9M tets)才轮到 `extract.blocks`; 整片 42 导体上云 |
| `lib/` | 模板库: `pad`、`xmon`、`cpw_meander`、`disc_transmon`、`bar_coupler`(各 = `.geo` 局部坐标几何 + `.yaml` 接口)+ `cpw_macros.geo`(include-guard 宏库; 宏名与入参一律 `LIB_`/`_lib_` 前缀 —— gmsh 的宏与变量都是进程级全局, 裸名会与用户自己的宏库重名、并把陈旧值漏给后续手写步骤) | 写自己的模板照抄: `.geo` 不打名、不定义 Macro, 输出变量在 yaml 里指名; `disc_transmon` 是可选外挂面(`if:` 开关)+ 多岛 + 浮动结的样板, `bar_coupler` 是连接型多岛(`body:`)+ 结的样板 | — |
