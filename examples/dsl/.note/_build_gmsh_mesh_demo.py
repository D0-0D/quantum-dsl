...
CELLS.append(code("""
from quantum_dsl.dsl.builder import build_ir

yaml_path = _REPO / "examples" / "dsl" / "yaml" / "chain_2q_native.metal.yaml"
...
# build_mesh options 走 _normalize_options 的 deep_merge: YAML 的 layer_stack /
# airbox / mesh 等同级 key 会原样保留, 只需把要新增 / 覆盖的那几段写出来即可
# (不必为了"共存"把整段 layer_stack 抄回来)。
result2 = build_mesh(
    yaml_path,
